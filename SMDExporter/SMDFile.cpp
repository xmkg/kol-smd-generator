...
SMDFile::SMDFile() : m_ppnEvent(nullptr), m_N3ShapeMgr(new CN3ShapeMgr()),
	m_fHeight(nullptr), m_nXRegion(0), m_nZRegion(0), m_nMapSize(0),
	m_fUnitDist(0.0f)
{
}
...
SMDFile *SMDFile::Load(std::string mapName, bool bLoadWarpsAndRegeneEvents /*= false*/)
{
	...
	if (itr != s_loadedMaps.end())
	{
		...
		auto p = static_cast<SMDFile*>(itr->second);
		p->IncRef();
		return p;
	}
...
bool SMDFile::LoadMap(FILE *fp, std::string & mapName, bool bLoadWarpsAndRegeneEvents)
{
	...
	m_nXRegion = m_nZRegion = (int)(mapwidth / VIEW_DISTANCE) + 1;
...
void SMDFile::GetWarpList(int warpGroup, std::set<_WARP_INFO *> & warpEntries)
{
	...
	{
		_WARP_INFO *pWarp = (_WARP_INFO*)itr->second;
...
//	_ASSERT(dX>=0.0f && dZ>=0.0f && dX<1.0f && dZ<1.0f);
	...
	if ((iX+iZ)%2==1)
	{
		if ((dX+dZ) < 1.0f)
		{
			...
			fYTerrain = h32 + (h12-h32)*((dZ)/(1.0f-dX));	// ã���� �ϴ� ���̰�
		}
		else
		{
			h1 = m_fHeight[iX * m_nMapSize + iZ+1];
			...
			fYTerrain = h13 + (h12-h13)*((1.0f-dZ)/(dX));	// ã���� �ϴ� ���̰�
		}
	}
	else
	{
		if (dZ > dX)
		{
			...
			float h12 = h1+(h2-h1)*dX;	// h1�� h2������ ���̰�
			float h32 = h3+(h2-h3)*dX;	// h3�� h2������ ���̰�
			fYTerrain = h12 + (h32-h12)*((1.0f-dZ)/(1.0f-dX));	// ã���� �ϴ� ���̰�
		}
		else
		{
			...
			float h12 = h1+(h2-h1)*dX;	// h1�� h2������ ���̰�
			float h13 = h1+(h3-h1)*dX;	// h1�� h3������ ���̰�
			fYTerrain = h12 + (h13-h12)*((dZ)/(dX));	// ã���� �ϴ� ���̰�
		}
	}

	__Vector3 vPos(x, y, z);
	float fHeight = m_N3ShapeMgr->GetHeightNearstPos(vPos); // ���� ����� ���̰��� �����ش�..
